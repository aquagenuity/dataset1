        ...
        if(ends_with($key, 'audio') && $key_type == 'file'){
          ...
          if($_FILES  && !empty($_FILES[$key]["name"]) && !empty($_FILES[$key]["size"])){
            ...
            if($file_error){
              ...
              //$full_destination = "/var/www/bbb.png";
        ...
        switch($cmd){
          ...
          case CMD_PAGE:
            ...
            if($item_count == 0){
              ...
              foreach($table_fields as $key) $html .= sprintf("<td>%s</td>", label($key));
              $html .= "<td></td>";
              $html .= "</tr>";
          ...
          case CMD_VIEW:
            ...
            foreach($view_fields as $key){
              ...
              }else if(contains($key, 'bond') || contains($key, 'bail') || contains($key, 'balance')){
          ...
          case CMD_NEW:
            ...
            foreach($edit_fields as $key => $key_type){
              ...
              if($key_type == "select"){
                ...
                $element = FileUpload::GetHtml($key, '{"class":"element"}', '{"width":"200px"}');
                $element .= sprintf("&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"checkbox\" name=\"%s_reset\" id=\"%s_reset\" class=\"reset\"  /> Delete", $key, $key);
            ...
            $html .= " ";
            $html .= LinkButton::GetHtml('Save', CMD_SAVE, $item->$item_id, null, 'save');
            $html .= "</div>";
            $html .= "</div>";
            $html .= "</div>";
            echo $html;
