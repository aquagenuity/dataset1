...
class UserController extends BaseController
{
	...
	public function password(){
		...
		if($post_back && !$double_post_back && $cmd == CMD_SUBMIT){
			$email = $_POST['email'];
	...
	public function profile(){
		...
		if($post_back && !$double_post_back && $cmd == CMD_SAVE){
			if($prof){
	...
	public function reset($id){
		...
		$user_id = Blowfish::Decrypt(base64_decode($id));
		$user = User::GetById($user_id);
		...
		if($post_back && !$double_post_back && $cmd == CMD_SUBMIT){
			...
			if($user){
				$password = $_POST['password'];
				$user->password = Blowfish::Encrypt($password);
				$user->Save();
